 ...
 * because Better Auth handles its own request body.
 */
app.all(
  "/api/auth/*",
  toNodeHandler(auth)
 ...
 * Stripe webhook
 *
 * Stripe signature validation requires the raw request body,
 * so this route must remain before express.json().
 */
app.post(
...
app.get("/", (_req, res) => {
  res.status(200).json({
    success: true,
    message: "CrowdSpark API is running",
    version: "1.0.0",
